...
export function InjectX<T>(constructor: Constructor<T>): {
  [K in Extract<keyof T, InjectMethod> as K extends InjectMethod<infer R> ? R : never]: T[K]
} {
        ...
        return createInjectBrandObject({
          ...
          getter: (r) => {
            ...
            if (typeof r[fnName] !== 'function') {
              throw new Error(
                `\`InjectX<${constructor.name}>.${name}' (\`${constructor.name}.prototype.${fnName}') doesn't exist.`,
              )
            }
